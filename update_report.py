...
def generate_html(data, recs, earnings):
    ...
    index_cards = "".join([f'''
        <div class="bg-white dark:bg-[#1e1e1e] p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm" onmouseenter="showChart('{s}')">
            <p class="text-[10px] text-zinc-500 font-bold mb-1">{data[s]['name']}</p>
            <p class="text-lg font-black italic text-zinc-900 dark:text-white">{data[s]['price']:,.1f}</p>
            <p class="{'text-red-500' if data[s]['chg'] > 0 else 'text-blue-500'} text-[11px] font-bold">{data[s]['chg']:+.2f}%</p>
    ...
    for day in earnings:
        ...
        for c in day['companies']:
            border = "border-[#D4AF37] bg-[#D4AF37]/5" if c['rec'] else "border-zinc-100 dark:border-zinc-800"
            badge = '<span class="bg-[#D4AF37] text-black text-[8px] px-1 rounded font-black ml-2">LONG</span>' if c['rec'] else ""
            earnings_html += f'''
            <div class="flex items-center justify-between p-3 rounded-xl border {border} mb-3 shadow-sm">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-xs font-black text-zinc-900 dark:text-white">{c['n']}{badge}</p>
                        <p class="text-[10px] text-zinc-500 font-bold">{c['s']}</p>
                    </div>
                </div>
    ...
    <head>
        ...
        <title>연신내 개미펀드 V17</title>
        <script src="https://cdn.tailwindcss.com"></script>
        <script src="https://cdn.jsdelivr.net/npm/echarts@5.4.3/dist/echarts.min.js"></script>
        <style>
            @import url('https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@400;700;900&display=swap');
            body {{ font-family: 'Noto Sans KR', 'Malgun Gothic', sans-serif; transition: background-color 0.3s; }}
            ...
            <header class="flex flex-col md:flex-row justify-between items-center mb-12 border-b border-zinc-200 dark:border-zinc-900 pb-10 gap-8">
                <div class="flex items-center gap-8">
                    ...
                    <div>
                        ...
                        <p class="text-[#D4AF37] font-black text-[10px] tracking-[0.4em] opacity-60 uppercase">15 Agents Elite Strategic Terminal</p>
                    </div>
                </div>
